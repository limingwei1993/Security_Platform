...
{
  int i;
  /* call etharp_tmr_EMAC often enough to have all entries cleaned */
  for(i = 0; i < 0xff; i++) {
    etharp_tmr_EMAC();
  }
}
...
default_netif_init(struct netif *netif)
{
  ...
  netif->output = etharp_output_EMAC;
  netif->mtu = 1500;
...
default_netif_add(void)
{
  ...
  netif_set_default_EMAC(netif_add_EMAC(&test_netif, &test_ipaddr, &test_netmask,
                              &test_gw, NULL, default_netif_init, NULL));
  netif_set_up_EMAC(&test_netif);
}
...
create_arp_response(ip_addr_t *adr)
{
  ...
  struct pbuf *p = pbuf_alloc_EMAC(PBUF_RAW, sizeof(struct eth_hdr) + sizeof(struct etharp_hdr), PBUF_RAM);
  if(p == NULL) {
    FAIL_RET();
  }
  ...
  ethernet_input_EMAC(p, &test_netif);
}
...
START_TEST(test_etharp_table)
{
  ...
  if (pcb != NULL) {
    ...
    for(i = 0; i < ARP_TABLE_SIZE; i++) {
      struct pbuf *p = pbuf_alloc_EMAC(PBUF_TRANSPORT, 10, PBUF_RAM);
      fail_unless(p != NULL);
      if (p != NULL) {
        err_t err = udp_sendto_EMAC(pcb, p, &adrs[i], 123);
        ...
        idx = etharp_find_addr_EMAC(NULL, &adrs[i], &unused_ethaddr, &unused_ipaddr);
        fail_unless(idx == i);
        etharp_tmr_EMAC();
      }
    }
    ...
    fail_unless(err == ERR_OK);
    idx = etharp_find_addr_EMAC(NULL, &adrs[ARP_TABLE_SIZE], &unused_ethaddr, &unused_ipaddr);
    fail_unless(idx == 0);
    ...
    idx = etharp_find_addr_EMAC(NULL, &adrs[ARP_TABLE_SIZE], &unused_ethaddr, &unused_ipaddr);
    fail_unless(idx == 0);
    idx = etharp_find_addr_EMAC(NULL, &adrs[ARP_TABLE_SIZE+1], &unused_ethaddr, &unused_ipaddr);
    fail_unless(idx == 2);
    /* and remove it again */
