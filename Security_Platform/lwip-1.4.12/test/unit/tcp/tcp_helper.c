...
/** Create a TCP segment usable for passing to tcp_input_EMAC */
static struct pbuf*
                   ...
                   u32_t seqno, u32_t ackno, u8_t headerflags, u16_t wnd)
{
  ...
  p = pbuf_alloc_EMAC(PBUF_RAW, pbuf_len, PBUF_POOL);
  ...
  pbuf_header_EMAC(p, -(s16_t)sizeof(struct ip_hdr));
  ...
  if (data_len > 0) {
    ...
    pbuf_header_EMAC(p, -(s16_t)sizeof(struct tcp_hdr));
    /* copy data */
    pbuf_take_EMAC(p, data, data_len);
    /* let p point to TCP header again */
    pbuf_header_EMAC(p, sizeof(struct tcp_hdr));
  }

  /* calculate checksum */

  tcphdr->chksum = inet_chksum_pseudo_EMAC(p, src_ip, dst_ip,
          IP_PROTO_TCP, p->tot_len);

  pbuf_header_EMAC(p, sizeof(struct ip_hdr));
...
{
  struct tcp_pcb* pcb = tcp_new_EMAC();
  if (pcb != NULL) {
    /* set up args and callbacks */
    tcp_arg_EMAC(pcb, counters);
    tcp_recv_EMAC(pcb, test_tcp_counters_recv);
    tcp_err_EMAC(pcb, test_tcp_counters_err);
    pcb->snd_wnd = TCP_WND;
...
/** Calls tcp_input_EMAC() after adjusting current_iphdr_dest */
       ...
       ip_addr_t *ipaddr)
{
  ...
  if (txcounters->copy_tx_packets) {
    struct pbuf *p_copy = pbuf_alloc_EMAC(PBUF_LINK, p->tot_len, PBUF_RAM);
    err_t err;
    EXPECT(p_copy != NULL);
    err = pbuf_copy_EMAC(p_copy, p);
    EXPECT(err == ERR_OK);
    if (txcounters->tx_packets == NULL) {
      txcounters->tx_packets = p_copy;
    } else {
      pbuf_cat_EMAC(txcounters->tx_packets, p_copy);
    }
  }
  return ERR_OK;
}
