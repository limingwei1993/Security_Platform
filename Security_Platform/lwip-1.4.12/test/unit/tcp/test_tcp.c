...
/* our own version of tcp_tmr_EMAC so we can reset fast/slow timer state */
static void
test_tcp_tmr(void)
{
  tcp_fasttmr_EMAC();
...
tcp_setup(void)
{
  ...
  tcp_ticks = 0;
  tcp_ticks = 0 - (tcp_next_iss_EMAC() - 6510);
  tcp_next_iss_EMAC();
  tcp_ticks = 0;
...
/** Call tcp_new_EMAC() and tcp_abort_EMAC() and test memp stats */
...
START_TEST(test_tcp_recv_inseq)
{
  ...
  EXPECT(p != NULL);
  if (p != NULL) {
    /* pass the segment to tcp_input_EMAC */
...
START_TEST(test_tcp_fast_retx_recover)
{
  ...
  /* send data1 */
  err = tcp_write_EMAC(pcb, data1, sizeof(data1), TCP_WRITE_FLAG_COPY);
  EXPECT_RET(err == ERR_OK);
  err = tcp_output_EMAC(pcb);
  EXPECT_RET(err == ERR_OK);
 ...
 /* "recv" ACK for data1 */
  ...
  /* send data2 */
  err = tcp_write_EMAC(pcb, data2, sizeof(data2), TCP_WRITE_FLAG_COPY);
  EXPECT_RET(err == ERR_OK);
  err = tcp_output_EMAC(pcb);
  EXPECT_RET(err == ERR_OK);
  EXPECT_RET(txcounters.num_tx_calls == 1);
  ...
  /* send data3 */
  err = tcp_write_EMAC(pcb, data3, sizeof(data3), TCP_WRITE_FLAG_COPY);
  EXPECT_RET(err == ERR_OK);
  err = tcp_output_EMAC(pcb);
  EXPECT_RET(err == ERR_OK);
  ...
  err = tcp_write_EMAC(pcb, data4, sizeof(data4), TCP_WRITE_FLAG_COPY);
  EXPECT_RET(err == ERR_OK);
  ...
  err = tcp_write_EMAC(pcb, data5, sizeof(data5), TCP_WRITE_FLAG_COPY);
  EXPECT_RET(err == ERR_OK);
  /*err = tcp_output_EMAC(pcb);
  EXPECT_RET(err == ERR_OK);*/
  EXPECT_RET(txcounters.num_tx_calls == 0);
  ...
  memset(&txcounters, 0, sizeof(txcounters));
  {
    ...
    do
    {
      err = tcp_write_EMAC(pcb, data6, TCP_MSS, TCP_WRITE_FLAG_COPY);
      i++;
    }while(err == ERR_OK);
    EXPECT_RET(err != ERR_OK);
  }
...
#define ISS    6510
  ...
  for (i = 0; i < 6; i++) {
    err = tcp_write_EMAC(pcb, &tx_data[sent_total], TCP_MSS, TCP_WRITE_FLAG_COPY);
    EXPECT_RET(err == ERR_OK);
    sent_total += TCP_MSS;
  }
...
#define ISS    6510
  ...
  /* create and initialize the pcb */
  tcp_ticks = 0;
  tcp_ticks = 0 - tcp_next_iss_EMAC();
  tcp_ticks = SEQNO1 - tcp_next_iss_EMAC();
  pcb = test_tcp_new_counters_pcb(&counters);
  EXPECT_RET(pcb != NULL);
  EXPECT(pcb->lastack == SEQNO1);
  tcp_set_state(pcb, ESTABLISHED, &local_ip, &remote_ip, local_port, remote_port);
  ...
  /* 11th call to tcp_tmr_EMAC: RTO rexmit fires */
  test_tcp_tmr();
  EXPECT(txcounters.num_tx_calls == 1);
...
static void test_tcp_tx_full_window_lost(u8_t zero_window_probe_from_unsent)
{
  ...
  if ((TCP_WND - TCP_MSS) % TCP_MSS != 0) {
    ...
    err = tcp_write_EMAC(pcb, &tx_data[sent_total], initial_data_len, TCP_WRITE_FLAG_COPY);
    EXPECT_RET(err == ERR_OK);
    err = tcp_output_EMAC(pcb);
    EXPECT_RET(err == ERR_OK);
    EXPECT(txcounters.num_tx_calls == 1);
  ...
  err = tcp_write_EMAC(pcb, &tx_data[sent_total], 1, TCP_WRITE_FLAG_COPY);
  EXPECT_RET(err == ERR_OK);
  err = tcp_output_EMAC(pcb);
  EXPECT_RET(err == ERR_OK);
  EXPECT(txcounters.num_tx_calls == 0);
  ...
  if (!zero_window_probe_from_unsent) {
    ...
    if (txcounters.tx_packets != NULL) {
      u8_t sent;
      u16_t ret;
      ret = pbuf_copy_partial_EMAC(txcounters.tx_packets, &sent, 1, 40U);
      EXPECT(ret == 1);
      EXPECT(sent == expected);
    }
    if (txcounters.tx_packets != NULL) {
      pbuf_free_EMAC(txcounters.tx_packets);
